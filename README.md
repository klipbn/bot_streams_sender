# Телеграм-бот для оповещения о стримах

## Общая информация

Проект представляет собой телеграм-бота, предназначенного для оповещения о текущих онлайн-трансляциях на платформах YouTube и Twitch. Бот собирает данные об онлайн-трансляциях, трансформирует их, загружает в базу данных PostgreSQL и отправляет оповещения в телеграм-канал. Процесс ETL (Extract, Transform, Load) реализован с использованием Apache Airflow.
...
### Обозначение значков
- ✔️ онлайн выше верхней границы доверительного интервала.
- 🔸 говорит о том, что онлайн находится в доверительном интервале.
- ✖️ онлайн ниже нижней границы доверительного интервала.
...
Данный проект позволяет в реальном времени отслеживать активность стримеров на популярных платформах и оперативно уведомлять аудиторию об интересующих трансляциях
